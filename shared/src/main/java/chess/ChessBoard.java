...
public class ChessBoard {
    ...
    public ChessBoard(ChessBoard board) {
        for (int row = 1; row <= 8; row++) {
            for (int col = 1; col <= 8; col++) {
                ...
                this.board[row-1][col-1] = board.getPiece(pos);
            }
        }
    }
    ...
    public boolean isOOB(ChessPosition pos){
    ...
    public boolean equals(Object other) {
        ChessBoard newBoard = (ChessBoard) other;
        for (int row = 1; row <= 8; row++) {
            for (int col = 1; col <= 8; col++) {
                ChessPosition newPos = new ChessPosition(row, col);
                if (hasPiece(newPos) && newBoard.hasPiece(newPos)) {
                    if (!getPiece(newPos).equals(newBoard.getPiece(newPos))) {
                        return false;
                    }
                }
                if (hasPiece(newPos) != newBoard.hasPiece(newPos)) {
